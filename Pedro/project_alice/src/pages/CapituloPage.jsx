...
function CapituloPage() {
    ...
    // REMOVIDO: const { token } = useAuth(); // Não é mais necessário
    
    const [pages, setPages] = useState([]);
    const [currentPageIndex, setCurrentPageIndex] = useState(0);
    const [loading, setLoading] = useState(true);
    const [isConcluido] = useState(false); // Mantido, mas não usado na lógica de conclusão de API
    ...
    // ----------------------------------------------------
    // 1. Lógica de Busca de Dados (ATUALIZADA)
    // ----------------------------------------------------
    useEffect(() => {
        const fetchPages = async () => {
            // REMOVIDO: if (!token) return; // Não há mais verificação de token

            try {
                // A requisição agora não precisa de cabeçalhos de autorização
                ...
                // Define um erro mais informativo caso a API falhe
                setError(`Não foi possível carregar o conteúdo do capítulo ${idCapitulo}. Verifique a conexão com a API.`);
            } finally {
                setLoading(false);
            }
        };
        ...
        fetchPages();
    }, [idCapitulo]); // O array de dependências não inclui mais o 'token'

    /*
    // ----------------------------------------------------
    // Lógica de Conclusão (REMOVIDA/COMENTADA)
    // ----------------------------------------------------
    // A função handleConcluirCapitulo é comentada, pois dependia do token.
    // Se o recurso de conclusão for necessário para usuários autenticados,
    // ele deve ser reintroduzido com uma verificação de `isAuthenticated` do contexto.
    
    const handleConcluirCapitulo = async () => {
        if (isConcluido || !token) return; 
        // ... lógica axios de patch ...
        // ...
    ...
    */
    ...
    // Esta lógica não requer login, apenas redireciona para o Quiz no final.
    const goToNextPage = () => {
        if (currentPageIndex < totalPages - 1) {
            setCurrentPageIndex(currentPageIndex + 1);
        } else {
            // REDIRECIONA PARA A PÁGINA DO QUIZ ao invés de concluir
            // Nota: O acesso ao quiz pode exigir login dependendo da sua rota de quiz.
