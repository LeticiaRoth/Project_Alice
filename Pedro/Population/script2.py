...
with pdfplumber.open(pdf_path) as pdf:
    for page in pdf.pages:
        ...
        while i < len(linhas):
            ...
            # Detecta título de capítulo com possíveis quebras de linha
            combinacoes = [linha]
            if i + 1 < len(linhas):
            ...
            if cap_encontrado:
                ...
                # Pula linhas do título detectado
            ...
            # Detecta início de parágrafo inteligente
            if linha and not linha.isdigit() and "Alice no País" not in linha:
                if linha[0].isupper() or linha.startswith("-"):
                ...
                # linha vazia ou cabeçalho indica fim do parágrafo
                if buffer_paragrafo:
                    paragrafo = " ".join(buffer_paragrafo).strip()
                    cursor.execute(
                        "INSERT INTO pagina (texto, numero, id_capitulo) VALUES (%s, %s, %s)",
